(() => {
  ...
  async function load(){
    try{
      // Read the public calendar alongside the signed-in member's booking lookup.
      // A slow booking read must not add a second round trip after the calendar.
      const [events,account]=await Promise.all([
        F.request(client.from('events').select('*').in('status',['scheduled','cancelled','completed']).order('event_date')),
        (async()=>{const auth=await F.request(client.auth.getSession());
          if(!auth.session)return {auth,rsvps:[],bookingError:false};
          try{return {auth,rsvps:await F.request(client.from('rsvps').select('event_id,status,payment_status,is_course_member').eq('member_id',auth.session.user.id)),bookingError:false};}
          catch{return {auth,rsvps:[],bookingError:true};}
        })()
      ]);
      const {auth,rsvps,bookingError}=account;
