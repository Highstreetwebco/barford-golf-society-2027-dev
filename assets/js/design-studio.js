(() => {
 ...
 function render(navigate=true){
  ...
  frame.className=phone?'phone':'desktop';if(new URLSearchParams(location.search).get('compact')==='1'){frame.style.width=phone?'360px':'100%';frame.style.height=phone?'640px':'830px';}frame.title=`${theme[0].toUpperCase()+theme.slice(1)} design — ${phone?'phone':'desktop'} preview`;
