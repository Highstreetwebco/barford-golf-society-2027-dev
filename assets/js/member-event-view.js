(() => {
  ...
  function render(host,model,{compact=false}={}) {
    ...
    const playerName=p=>p.full_name||p.guest_name||'Guest';
    const avatar=(p,index)=>{
      const name=playerName(p),initials=name.trim().split(/\s+/).slice(0,2).map(part=>part[0]).join('').toUpperCase();
      return p.photo_url?`<button type="button" class="group-photo" data-group-photo="${index}" aria-label="Enlarge ${esc(name)}’s profile photo" disabled><span aria-hidden="true">${esc(initials)}</span></button>`:`<span class="group-photo group-photo-placeholder" aria-label="No profile photo for ${esc(name)}"><span aria-hidden="true">${esc(initials)}</span></span>`;
    };
    ...
    host.innerHTML=`${event.test_mode_active?'<p class="simple-notice"><strong>Practice event</strong> — this is a test round.</p>':''}
      ${dashboardCard?`<section class="simple-card" id="dashboardScorecard" aria-labelledby="dashboardScorecardTitle">
        <div class="group-camera-heading"><div><p class="eyebrow">${matchDay?'Playing today':'Tee groups announced'}</p><h2 id="dashboardScorecardTitle">${matchDay?'Today’s group':'Your tee group'}</h2></div>${matchDay?`<button id="dashboardEventCamera" class="button button-outline group-camera-button" type="button" aria-label="Take a photo for the event gallery" title="Take a photo for the event gallery"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M4 6h4l2-3h4l2 3h4a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2Z"/><circle cx="12" cy="13" r="4"/></svg></button>`:''}</div>
        ${matchDay?`<input id="dashboardEventCameraInput" hidden type="file" accept="image/*" capture="environment"><p id="eventPhotoStatus" role="status" aria-live="polite"></p><button id="eventPhotoRetry" type="button" class="button button-outline hidden">Retry photo upload</button><a id="eventPhotoGallery" class="simple-back hidden" href="gallery.html?event=${encodeURIComponent(event.id)}">View event photos →</a>`:''}
        <p><strong>${esc(event.name)}</strong>${tee?` · Tee off ${esc(F.time(tee))}`:''}</p>
        <ul class="simple-player-list dashboard-group-players">${group.map((p,index)=>`<li class="tee-group-player">${avatar(p,index)}<span>${esc(playerName(p))}${p.is_you||p.member_id===session.user.id?' (You)':''}</span></li>`).join('')}</ul>
    ...
    const pictured=group.map((p,index)=>({...p,element:host.querySelector(`[data-group-photo="${index}"]`)})).filter(p=>p.photo_url&&p.element);
    // Paint names and initials immediately; fetch all authorised image links once.
    if(pictured.length)F.request(window.BarfordSupabase.storage.from('profile-images').createSignedUrls([...new Set(pictured.map(p=>p.photo_url))],3600)).then(rows=>{
      ...
      for(const p of pictured){
        const photo=p.element,url=urls.get(p.photo_url);if(!url||!photo.isConnected)continue;
        const img=document.createElement('img');img.alt=playerName(p);img.decoding='async';
        img.onload=()=>{if(!photo.isConnected)return;photo.replaceChildren(img);photo.dataset.profilePhoto=url;photo.disabled=false;};
        img.onerror=()=>photo.setAttribute('aria-label',`Photo unavailable for ${playerName(p)}`);
        img.src=url;
      }
    }).catch(()=>{});
