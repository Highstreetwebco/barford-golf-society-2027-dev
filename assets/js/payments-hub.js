(() => {
  ...
  async function load(){
    try{
      ...
      const receipt=e=>{const r=rsvps.find(x=>x.event_id===e.id),pay=F.payment(e,r);return `<article class="simple-card payment-receipt"><p class="eyebrow">${F.esc(F.date(e.event_date))}</p><h2>${F.esc(e.name)}</h2><p>${F.esc(F.priceLabel(r))}</p><p class="simple-payment-state"><strong>${F.esc(pay.label)}</strong></p>${pay.due?'<p>Online payments are coming soon. Pay now opens a demonstration; no money is taken. Use Get payment instructions to arrange a real payment.</p>':e.status==='cancelled'&&r?.payment_status==='paid'?'<p>Contact the committee to check your refund.</p>':r?.status==='reserve'?'<p>No payment is needed while you wait for a place.</p>':''}${pay.due?`<button type="button" class="button button-primary full-button" data-pay-now="${F.esc(e.id)}">Pay now · ${F.esc(F.money(F.eventPrice(e,r)))}</button><button type="button" class="button button-outline full-button" data-payment-help="${F.esc(e.id)}">Get payment instructions</button><details class="simple-details"><summary>Already sent your payment?</summary><p>It has not been confirmed yet. Ask the committee to check before paying again.</p><button class="button button-outline" data-payment-check="${F.esc(e.id)}">Ask for confirmation</button></details>`:''}${r?.status==='playing'&&e.status==='scheduled'?`<a class="button button-outline full-button" href="${F.eventUrl(e.id)}#my-group">View my tee group →</a><p>Your tee time and group appear there when announced.</p>`:''}<a class="simple-back" href="${F.eventUrl(e.id)}">Back to this event</a></article>`;};
      ...
      host.querySelectorAll('[data-pay-now]').forEach(b=>b.onclick=()=>window.BarfordPayments.pay(events.find(e=>e.id===b.dataset.payNow),rsvps.find(r=>r.event_id===b.dataset.payNow),b));
