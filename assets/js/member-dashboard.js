(() => {
  ...
  async function load(){
    ...
    try{
      ...
      const [events,recent,rsvps]=await Promise.all([
        ...
        F.request(client.from('rsvps').select('id,event_id,status,payment_status,is_course_member,buggy_requested,preferred_tee_time').eq('member_id',auth.session.user.id))
      ]);
