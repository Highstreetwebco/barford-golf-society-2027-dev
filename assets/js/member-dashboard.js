(() => {
  ...
  async function load(){
    ...
    try{
      ...
      const events=await F.request(client.from('events').select('id,event_date').eq('status','scheduled').gte('event_date',F.today()).order('event_date').order('first_tee_time'));
      ...
      const rsvps=await F.request(client.from('rsvps').select('event_id,status').eq('member_id',auth.session.user.id).in('event_id',events.map(e=>e.id)));
      ...
      const model=await F.loadEvent(next.id);window.BarfordEventView.render(host,model,{compact:true});
    }catch(error){
